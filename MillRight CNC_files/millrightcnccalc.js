...
$(document).ready(function() {
	...
	function ensureCheckboxRadio() {
		$('input[type=radio]').checkboxradio();
	}

	function setUpLocalStorageFor(name) {
	...
	if (window.localStorage) {
		setUpLocalStorageFor('Units');
		setUpLocalStorageFor('Machine');
...
Materials.Aluminum[0] = [0.0007, 0.0009, 0.0010, 0.0011];
Materials.Aluminum[1] = [0.012, 0.018, 0.0175, 0.0175];

Materials.Hard_Plastic[0] = [0.0015, 0.002, 0.0023, 0.0027];
Materials.Hard_Plastic[1] = [0.05, 0.065, 0.075, 0.075];

Materials.Hardwood[0] = [0.0012, 0.0016, 0.002, 0.0023];
Materials.Hardwood[1] = [0.04, 0.06, 0.08, 0.08];

Materials.MDF[0] = [0.0018, 0.002, 0.0023, 0.0027];
Materials.MDF[1] = [0.08, 0.12, 0.16, 0.17];

Materials.Soft_Plastic[0] = [0.0016, 0.002, 0.0023, 0.0025];
Materials.Soft_Plastic[1] = [0.06, 0.07, 0.08, 0.08];

Materials.Softwood[0] = [0.0016, 0.0022, 0.0024, 0.0028];
Materials.Softwood[1] = [0.06, 0.1, 0.14, 0.16];
...
function calculate() { 
  ...
  if (Materials[materialChoice] != null) {
    feedRateInches = Math.min(320, Number(Materials[materialChoice][0][cutterChoice])*speedRate*numberOfFlutes*feedFactor)

    depthOfCut = (Number(Materials[materialChoice][1][cutterChoice])*Number(machineFactor)*Number(aggressionFactor));
    feedRate = Number(feedRateInches);
...
function writeValues(){
	"use strict";
		...
		$('#chipLoadArea').val(parseFloat(chipLoad).toFixed(4) + " in/tooth");
		...
		$('#chipLoadArea').val(parseFloat((chipLoad)*25.4).toFixed(4) + " mm/tooth");
...
function calcSFPM(){
	...
	var diam = eval(document.myForm1.spfm_1.value);
 	var rpm = eval(document.myForm1.spfm_2.value);
 	var sfpm = eval(document.myForm1.spfm_3.value);

	var cutSpeed = $("input[name=cutSpeed]:checked").val();
	...
	if (cutSpeed == "1") { 
		...
		var diam1 = parseFloat((sfpm*12/(Math.PI*rpm)).toFixed(4));
		document.myForm1.spfm_1.value = diam1;
	  }
	}
	...
	if (cutSpeed == "2") {
		  ...
		  if ( errorMsg != "" ) {
			...
			return;
				...
				var rpm1 = parseFloat((sfpm*12/(Math.PI*diam))).toFixed(0);
				document.myForm1.spfm_2.value = rpm1;
		}
	}
	...
	if (cutSpeed == "3") {
		  ...
		  if ( errorMsg != "" ) {
			...
			return;
				...
				var sfpm1 = parseFloat((((Math.PI*rpm*diam)/12)*100)/100).toFixed(4);
				document.myForm1.spfm_3.value = sfpm1;
...
function cutTime(){
	...
	wpl = $('input#wpl').val();
	clpm = $('input#clpm').val();
	ctm = parseFloat(wpl)/parseFloat(clpm);
/*	cts = ctm*60; */
	cuttingTime.value=(parseFloat(ctm).toFixed(4));
/*	document.myForm3.goukei33.value = ii; */
}